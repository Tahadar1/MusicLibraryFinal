...
public class MusicController {
    ...
    public ResponseEntity<List<UploadResponse>> getAllSongs() {
        List<UploadResponse> songs = musicService.getAllSongs().map(music -> {
            String url = ServletUriComponentsBuilder.fromCurrentContextPath().path("").path(String.valueOf(music.getFileName())).toUriString();
            return new UploadResponse(music.getMusic_Id(), music.getFileName(), url, music.getFileType(), music.getData().length, music.getIsFavorite());
    ...
    public ResponseEntity<UploadResponse> getMusicByFileName(@PathVariable("fileName") String filename) {
        ...
        String url = ServletUriComponentsBuilder.fromCurrentContextPath().path("").path(String.valueOf(music.getFileName())).toUriString();
        UploadResponse song = new UploadResponse(music.getMusic_Id(), music.getFileName(), url, music.getFileType(), music.getData().length, music.getIsFavorite());
